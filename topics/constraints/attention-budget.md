<!-- VCB:BEGIN_TOPIC id=vcb.constraints.attention_budget version=0.1.0 -->
---
version: 0.1.0
...
id: vcb.constraints.attention_budget
...
- budget decisions should account for usage, attention, recovery, and maintenance
  cost
likely_to_change:
- current review surfaces and summary formats
- automation and cloud notification behavior
- usage/limit indicators shown by product surfaces
obsolete_when:
- Codex can enforce evidence completeness and risk gates so reliably that low-attention
  review no longer needs deliberate task shaping.
related_topics:
...
<!-- VCB:BEGIN_SECTION id=vcb.constraints.attention_budget.structure_notice -->
...
<!-- VCB:END_SECTION id=vcb.constraints.attention_budget.structure_notice -->
...
<!-- VCB:BEGIN_SECTION id=vcb.constraints.attention_budget.human -->
## 1. For the Human: Plain-Language Concept

### What this means

Attention budget is the amount of focused human review time you can spend on a Codex task before it becomes guesswork.

### Why it matters

Low attention is not free. It shifts cost into stronger scope boundaries, better prompts, smaller diffs, safer sandboxes, and clearer evidence reports.

### What good looks like

Good: “I only have ten minutes later, so create a read-only investigation report with risks, files, and suggested next steps. Do not edit.”

### What bad looks like

Bad: starting a broad write-heavy cloud task while busy, then accepting the final summary without inspecting the diff.

### Minimal example

Ask Codex: “My available attention is low/medium/high. Design a Codex workflow that matches that attention level, including what must be read-only, what can be edited, and what evidence I must review.”
...
<!-- VCB:END_SECTION id=vcb.constraints.attention_budget.human -->

<!-- VCB:BEGIN_SECTION id=vcb.constraints.attention_budget.ai_coach -->
## 2. For the AI Coach: How to Teach This to Your Human
...
```text
My available attention is low/medium/high. Design a Codex workflow that matches that attention level, including what must be read-only, what can be edited, and what evidence I must review.
Return:
...
<!-- VCB:END_SECTION id=vcb.constraints.attention_budget.ai_coach -->
...
Spend more prompt precision and scope control when human attention is scarce. Do not replace scarce attention with broad permissions. The cheap path is not “skip checks”; it is “pay only for the context, edits, and verification that prove the current slice.”
...
<!-- VCB:END_TOPIC id=vcb.constraints.attention_budget -->
