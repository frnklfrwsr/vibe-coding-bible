<!-- VCB:BEGIN_TOPIC id=tool.supabase version=0.1.0 -->
---
version: 0.1.0
...
name: Supabase
type: tool_card
category: database_backend_auth
status: active
...
setup_effort: 'medium: a project can start quickly, but schema, RLS, auth boundaries, service
  keys, migrations, backups, and connection posture need explicit review'
maintenance_effort: 'high: database schema, policies, auth settings, migrations, backups,
  storage rules, functions, and dependency clients drift as the app grows'
debugging_effort: 'high: failures may involve SQL, RLS, auth sessions, generated clients,
  migrations, functions, storage policies, connection pooling, or environment secrets'
lock_in_risk: 'moderate-high: Postgres portability helps, but auth/storage/functions/dashboard
  conventions and policy setup can become platform-specific'
hidden_cost_risk: 'high: fast backend setup can hide migration, data-loss, access-control,
  backup, secret, and production-incident costs'
codex_integration_value: 'medium-high: useful as implementation or verification infrastructure,
  ...
  criteria'
best_for:
- Postgres-backed MVP backend
- small-team app database
- auth/storage/database prototype that may become real
- RLS-aware app data layer
- backend evidence before app-builder promotion
not_for:
- casual production data experiments
- client-exposed service-role secrets
- unreviewed RLS policies
- schema changes without backups/migrations
- stable prose about current quotas or plan behavior
integrates_with_codex:
- Vercel or app-builder frontend
- Clerk/Auth0/first-party auth comparison
- Stripe webhook backend
- Sentry/PostHog event context
- Codex Security and schema review
hidden_costs:
...
obsolete_when:
- Supabase is no longer available, no longer documented by its vendor, or no longer serves
  its stated database_backend_auth role.
related_topics:
- tool.vercel
- tool.clerk
- tool.auth0
...
> Summary:
> Supabase is a Postgres-backed database/backend/auth/storage platform for MVP and production-bound app data tool. Use it to make product infrastructure more explicit, not to skip design, review, security, or operational ownership.
...
<!-- VCB:BEGIN_SECTION id=tool.supabase.human -->
## 1. For the Human: Plain-Language Concept

### What this tool is

Supabase is for Postgres-backed database/backend/auth/storage platform for MVP and production-bound app data. It belongs in the tool stack when the project needs that infrastructure role to be explicit and reviewable.
...
|---|---|
| Setup effort | medium: a project can start quickly, but schema, RLS, auth boundaries, service keys, migrations, backups, and connection posture need explicit review |
| Maintenance effort | high: database schema, policies, auth settings, migrations, backups, storage rules, functions, and dependency clients drift as the app grows |
| Debugging effort | high: failures may involve SQL, RLS, auth sessions, generated clients, migrations, functions, storage policies, connection pooling, or environment secrets |
| Lock-in risk | moderate-high: Postgres portability helps, but auth/storage/functions/dashboard conventions and policy setup can become platform-specific |
| Hidden cost risk | high: fast backend setup can hide migration, data-loss, access-control, backup, secret, and production-incident costs |
...
<!-- VCB:END_SECTION id=tool.supabase.human -->

<!-- VCB:BEGIN_SECTION id=tool.supabase.ai_coach -->
## 2. For the AI Coach: How to Teach This to Your Human

### Teaching objective

Teach `tool.supabase` as a production-infrastructure tool card. The human should use it to expose risk, evidence, and ownership, not to outsource judgment to a vendor dashboard or generated integration.
...
<!-- VCB:END_SECTION id=tool.supabase.ai_coach -->

## Shortcut Reality

### The shortcut people take

They add Supabase because it appears to solve a whole infrastructure category quickly, then accept the default setup or generated integration as production evidence.
...
- `supabase.docs.overview` — official vendor documentation anchor.
- `supabase.docs.database` — official vendor documentation anchor.
- `supabase.docs.auth` — official vendor documentation anchor.
- `supabase.docs.edge_functions` — official vendor documentation anchor.
- `supabase.docs.secrets` — official vendor documentation anchor.
...
<!-- VCB:END_TOPIC id=tool.supabase -->
