---
id: vcb.index.by_stability
title: INDEX_BY_STABILITY
artifact_type: index
...
## Quarterly review — workflow/prompting topic cards
- `vcb.prompting.four_part_prompt` → active: stable prompt pattern, volatile Codex surface details
- `vcb.prompting.acceptance_criteria` → active: stable done-when pattern, volatile product check/report features
- `vcb.prompting.plan_first` → active: stable planning gate, volatile plan-mode/product affordances
- `vcb.prompting.context_management` → active: stable curation principle, volatile context-window/surface behavior
- `vcb.workflow.unknown_codebase` → active: stable map-before-mutation workflow, volatile exploration features
- `vcb.workflow.feature_slicing` → active: stable vertical-slice workflow, volatile execution affordances
- `vcb.workflow.bug_repro` → active: stable repro-first debugging, volatile debugging/browser features
- `vcb.workflow.testing` → active: stable executable-evidence principle, volatile test integration features

## Semiannual review — agentic operating principles
...
## Quarterly review — Codex feature families
...
## Monthly review — volatile Codex surfaces, commands, and economics
...
## Snapshot-only review — pricing and tool catalogs
- `vcb.constraints.plus_vs_pro_vs_api` → planned
- `vcb.tool_catalog.ai_coding_tools` → planned
- `vcb.tool_catalog.minimum_viable_stack` → planned
...
## Review quarterly: shortcut posture
...
## Highly volatile pricing and plan details
...
## Stable concept cards expanded in Chunk 13
- `vcb.concepts.api_basics` → annual
- `vcb.concepts.frontend_backend` → annual
- `vcb.concepts.database_schema` → annual
- `vcb.concepts.authentication` → annual
- `vcb.concepts.authorization` → annual
- `vcb.concepts.state` → annual
- `vcb.concepts.async` → annual
- `vcb.concepts.dependency` → annual
- `vcb.concepts.test` → annual
- `vcb.concepts.typecheck` → annual
- `vcb.concepts.lint` → annual
- `vcb.concepts.migration` → annual
- `vcb.concepts.environment_variable` → annual
- `vcb.concepts.build_step` → annual
- `vcb.concepts.ci` → annual
- `vcb.concepts.feature_flag` → annual
- `vcb.concepts.observability` → annual

## Codex feature cards requiring quarterly/monthly review
- `tool.codex_feature_maturity` → active: Codex maturity-label governance, feature-readiness posture, fallback requirements, and review cadence
- `tool.codex_changelog_monitoring` → active: official Codex release monitoring, targeted VCB maintenance, deprecation watch, and source/route refreshes
- `tool.codex_security` → active: Codex Security plugin/cloud scans, security diff review, finding validation, threat-model-informed triage, and reviewed fix preparation
- `tool.codex_agents_md` → active: durable repo/project guidance for repeated Codex expectations and local conventions
...
- `vcb.codex.app` → active: quarterly review
- `vcb.codex.cli` → active: quarterly review
- `vcb.codex.ide_extension` → active: quarterly review
- `vcb.codex.cloud` → active: quarterly review
- `vcb.codex.github_review` → active: quarterly review
- `vcb.codex.config` → active: quarterly review
- `vcb.codex.agents_md` → active: quarterly review
- `vcb.codex.skills` → active: quarterly review
- `vcb.codex.mcp` → active: quarterly review
- `vcb.codex.hooks` → active: quarterly review
- `vcb.codex.automations` → active: quarterly review
- `vcb.codex.subagents` → active: quarterly review
- `vcb.codex.computer_use` → active: quarterly review
- `vcb.codex.feature_maturity` → active companion: monthly review
- `vcb.codex.changelog_monitoring` → active companion: monthly review
...
## Stability routes for frontend, refactor, dependency, release, and documentation workflows
...
## Quarterly review — active failure-mode cards
- `vcb.failure.hallucinated_apis` → active: stable contract-evidence principle; volatile vendor APIs and SDKs
- `vcb.failure.context_pollution` → active: stable context-curation principle; volatile context windows and surface behavior
- `vcb.failure.weakened_tests` → active: stable executable-evidence principle; volatile test frameworks and snapshot tooling
- `vcb.failure.broad_refactor_drift` → active: stable behavior-preservation principle; volatile framework migration patterns
- `vcb.failure.dependency_bloat` → active: stable maintenance-cost principle; volatile package ecosystems and advisories
- `vcb.failure.ui_taste_gap` → active: stable user-state/evidence principle; volatile UI tooling and design systems
- `vcb.failure.ci_false_confidence` → active: stable configured-check humility; volatile CI providers and pipeline defaults
- `vcb.failure.done_claim_without_evidence` → active: stable evidence-before-acceptance principle; volatile agent reporting formats

## Quarterly/monthly review — active constraint and budget cards

- `vcb.constraints.operating_mode` → quarterly: durable principle, volatile Codex surfaces and plan packaging
- `vcb.constraints.attention_budget` → quarterly: durable attention model, volatile review surfaces
- `vcb.constraints.recovery_budget` → quarterly: durable recoverability model, volatile product rollback affordances
- `vcb.constraints.build_vs_maintenance` → quarterly: durable phase model, volatile product features
- `vcb.constraints.production_quality` → quarterly: durable release-readiness model, volatile CI/review surfaces
- `vcb.constraints.scope_budget` → monthly/quarterly: stable workflow posture, highly volatile pricing surface
- `vcb.constraints.review_budget` → monthly/quarterly: stable throughput tradeoff, volatile cloud/subagent limits
- `vcb.constraints.usage_burn` → monthly: exact limits, credits, and pricing are highly volatile

## Quarterly Review — Active Shortcut Harm-Reduction Cards
...
## Quarterly Review — Security and Permission Shortcut Cards
...
## Quarterly setup/config/process shortcut review

- `vcb.shortcut.skipping_setup` → active: review when setup commands, package manager, or test/build path changes
- `vcb.shortcut.default_config_forever` → active: review when sandbox, approval, config, or project phase changes
- `vcb.shortcut.skipping_agents_md` → active: review when repeated project guidance is missing from durable repo instructions
- `vcb.shortcut.overstuffing_agents_md` → active: review when persistent guidance grows noisy, stale, or task-specific
- `vcb.shortcut.stale_agents_md` → active: review after repo commands, architecture, ownership, or CI changes
- `vcb.shortcut.unofficial_tools` → active: review when MCP/plugin/tool ecosystems, auth, or marketplace trust changes
- `vcb.shortcut.hook_overreach` → active: review when hook events, trust model, or managed-policy behavior changes
- `vcb.shortcut.trusting_external_tool_output` → active: review when external tool provenance, citation, or cache/live behavior changes

## Quarterly review routes for tool-sprawl and process shortcuts
...
## Stability routes for multi-AI/model-bias shortcuts
...
## Repository, CI, dependency, and QA tool-card review cadence

- `tool.github` → active: quarterly review; repository settings, branch rules, PR UI, and collaboration features are volatile.
- `tool.github_actions` → active: quarterly review; runner images, workflow syntax, token permissions, and billing are volatile.
- `tool.github_dependabot` → active: quarterly review; update behavior, advisory metadata, grouping, and ecosystem support are volatile.
- `tool.npm` → active: quarterly review; package-manager commands, lifecycle scripts, lockfile behavior, and audit output are volatile.
- `tool.playwright` → active: quarterly review; browser versions, CI setup, test runner behavior, and artifact formats are volatile.
- `tool.openssf_scorecard` → active: quarterly review; checks, scoring mechanics, action setup, and policy guidance are volatile.

## Quarterly review — AI coding, AI IDE, and planning tool cards
...
- Review pricing, model, feature, IDE, permission, context, and enterprise/admin mechanics against official vendor docs.


## Stability routes for local-dev, design, and project-management tools

- `tool.docker` → active: Dockerfiles, Compose/build/runtime behavior, security defaults, licensing/pricing, and Desktop/Engine packaging require source-checked review
- `tool.figma` → active: feature availability, Dev Mode/export/handoff mechanics, pricing, seats, and integration behavior require source-checked review
- `tool.linear` → active: workflow/status mechanics, integrations, automation behavior, pricing, seats, and planning defaults require source-checked review

## Quarterly review — browser-dev, app-builder, and UI-generation tools
...
- exact pricing, quotas, hosting/runtime limits, model availability, export behavior, deployment mechanics, UI labels, and current integrations require source checks

## Quarterly review — hosting, backend/auth, payment, observability, and analytics tools

- `tool.vercel` → quarterly; pricing, deployment behavior, runtime limits, UI labels, analytics/observability, and integration behavior are volatile.
- `tool.supabase` → quarterly; database limits, auth behavior, RLS guidance, backups, functions, secrets, and SDKs are volatile.
- `tool.clerk` → quarterly; auth/session/user/org behavior, SDKs, security controls, and feature packaging are volatile.
- `tool.auth0` → quarterly; IAM flows, tenant/app/API settings, organizations, SDKs, and enterprise identity behavior are volatile.
- `tool.stripe` → quarterly; checkout, billing, subscription, webhook, tax, and payment mechanics are volatile and money-sensitive.
- `tool.sentry` → quarterly; SDK, retention, tracing, replay, privacy, alerting, and AI/observability features are volatile.
- `tool.posthog` → quarterly; event, replay, feature-flag, experiment, analytics, retention, and pricing mechanics are volatile.

## Coverage-gap tools requiring source review

- `tool.cloudflare` → active: quarterly/monthly source check for edge runtime, Pages/Workers, DNS/security defaults, limits, and pricing.
- `tool.netlify` → active: quarterly source check for deploy contexts, functions, env vars, build/runtime behavior, limits, and pricing.
- `tool.neon` → active: quarterly source check for branches, connection/runtime behavior, auth/storage/functions availability, limits, and pricing.
- `tool.resend` → active: quarterly source check for email API/templates/domain behavior, deliverability docs, limits, and pricing.
- `tool.vitest` → active: quarterly source check for test APIs, browser/component testing, environment requirements, and migration changes.
- `tool.storybook` → active: quarterly source check for test runner, addons, builders, docs/testing behavior, and version mechanics.

<!-- VCB:STOP_RETRIEVAL reason="by_stability_complete" -->
<!-- VCB:END_INDEX id=vcb.index.by_stability -->
