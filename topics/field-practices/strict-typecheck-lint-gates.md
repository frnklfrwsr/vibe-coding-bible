<!-- VCB:BEGIN_TOPIC id=vcb.field.strict_typecheck_lint_gates version=1.0.1-post-release.issue7 -->
---
...
evidence_scope: bounded local reproduction in VCB validator workflow; official/vendor sources support check mechanics, but the practice is not promoted as universal guidance
budget_profiles:
- plus_constrained
- pro_high_throughput
- api_pay_as_you_go
...
id: vcb.field.strict_typecheck_lint_gates
title: Strict Typecheck/Lint Gates
type: field_practice
status: active
field_practice_status: reproduced
local_reproduction_basis: 'VCB validator gate was run before and after repository changes; bounded to this repo validator, not every lint/typecheck stack.'
...
durable_principles:
- Machine-verifiable constraints reduce uncertainty when agent output is plausible but unproven.
...
obsolete_when:
- The project toolchain changes, checks become flaky/noisy, or the gate stops correlating
  with actual defects.
related_topics:
...
# Strict Typecheck/Lint Gates

> Summary:
> Reproduced field practice (bounded): Require typecheck, lint, test, or CI gates after agent changes, and reject “done” claims that do not show the relevant command and result. This card labels the practice, gives safe trial conditions, and prevents the tactic from being treated as official best practice before evidence supports promotion.
...
<!-- VCB:BEGIN_SECTION id=vcb.field.strict_typecheck_lint_gates.human -->
## 1. For the Human: Plain-Language Concept

### What this practice is

Require typecheck, lint, test, or CI gates after agent changes, and reject “done” claims that do not show the relevant command and result.
...
Good: Codex reports the exact commands run, output summary, failures, skipped checks, and residual risk; noisy checks are fixed or explicitly scoped.

### What bad looks like

Bad: The team says “looks fine” because the generated code is clean, while type errors, lint drift, or broken tests remain unchecked.

### Safe trial

Apply strict gates to one repo slice with stable commands. Track false positives, runtime, and defects caught before making the gate durable.
...
<!-- VCB:END_SECTION id=vcb.field.strict_typecheck_lint_gates.human -->

<!-- VCB:BEGIN_SECTION id=vcb.field.strict_typecheck_lint_gates.ai_coach -->
## 2. For the AI Coach: How to Teach This to Your Human
...
Practice: Strict Typecheck/Lint Gates
Claim: Require typecheck, lint, test, or CI gates after agent changes, and reject “done” claims that do not show the relevant command and result.
...
<!-- VCB:END_SECTION id=vcb.field.strict_typecheck_lint_gates.ai_coach -->

## Shortcut Reality

### Ideal practice

Start from the durable principle: Machine-verifiable constraints reduce uncertainty when agent output is plausible but unproven.
...
- green gates are evidence, not full correctness proof
- bounded trial task
...
- The durable principle is stable: Machine-verifiable constraints reduce uncertainty when agent output is plausible but unproven.
...
- The project toolchain changes, checks become flaky/noisy, or the gate stops correlating with actual defects.
...
- Issue #7 audit: Bounded local reproduction is based on this repository's validator gate, which is a machine-verifiable check run before and after metadata changes.
...
<!-- VCB:END_TOPIC id=vcb.field.strict_typecheck_lint_gates -->
