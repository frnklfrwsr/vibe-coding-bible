<!-- VCB:BEGIN_TOPIC id=tool.netlify version=0.1.0 -->
---
version: 0.1.0
...
evidence_scope: official Netlify documentation anchors for deploys, functions, environment
  variables, and VCB maintainer synthesis for risk and coaching guidance.
budget_profiles:
- plus_constrained
- pro_high_throughput
- api_pay_as_you_go
...
setup_effort: 'medium: Git connection can be fast, but deploy contexts, build settings,
  functions, domains, and env vars need explicit review'
maintenance_effort: 'medium-high: deploy settings, functions, build images, env vars,
  domain configuration, and rollback paths drift'
debugging_effort: 'medium-high: failures can come from build config, deploy context,
  functions, env vars, redirects, forms, cache, or preview/production mismatch'
lock_in_risk: 'moderate: deploy contexts, functions, redirects, forms, environment
  behavior, and platform-specific config may shape the app'
hidden_cost_risk: 'medium-high: previews and functions hide costs in env management,
  migration, debugging, and release discipline'
codex_integration_value: 'medium-high: useful as evidence infrastructure or implementation
  ...
  acceptance criteria'
best_for:
- static or framework site deployment
- deploy previews and branch-context review
- serverless function prototypes with review gates
- environment-variable/context review
- rollback and deployment evidence for web changes
not_for:
- unreviewed production promotion
- unclear preview versus production context
- stable prose about limits, pricing, or UI labels
- secret-heavy experiments without account separation
- using deploy success as correctness proof
...
obsolete_when:
- Netlify is no longer available, no longer documented by its vendor, or no longer
...
> Summary:
> Netlify is for site deploys, deploy previews, functions, environment contexts, and rollback/deployment evidence. Use it to make evidence more explicit, not to skip design, review, security, or operational ownership.
...
<!-- VCB:BEGIN_SECTION id=tool.netlify.human -->
## 1. For the Human: Plain-Language Concept

### What this tool is

Netlify is a web hosting and deployment platform for sites, deploy previews, deploy contexts, functions, and Git-connected delivery workflows.

### Why it matters

It is a common alternative or companion to Vercel-style deployment. Codex work that changes deploy settings, functions, env vars, or preview behavior needs deploy evidence, not just a local pass.
...
|---|---|
| Setup effort | medium: Git connection can be fast, but deploy contexts, build settings, functions, domains, and env vars need explicit review |
| Maintenance effort | medium-high: deploy settings, functions, build images, env vars, domain configuration, and rollback paths drift |
| Debugging effort | medium-high: failures can come from build config, deploy context, functions, env vars, redirects, forms, cache, or preview/production mismatch |
| Lock-in risk | moderate: deploy contexts, functions, redirects, forms, environment behavior, and platform-specific config may shape the app |
| Hidden cost risk | medium-high: previews and functions hide costs in env management, migration, debugging, and release discipline |
...
<!-- VCB:END_SECTION id=tool.netlify.human -->

<!-- VCB:BEGIN_SECTION id=tool.netlify.ai_coach -->
## 2. For the AI Coach: How to Teach This to Your Human

### Coaching stance

Treat `tool.netlify` as a tool-selection and evidence-routing card. The goal is not to make the user use Netlify; it is to decide whether this surface is the smallest reliable tool for the job and what evidence is required before trusting it.
...
```text
Use `tool.netlify` only as a bounded tool in this project. First identify what it can touch, what evidence proves it is configured or used correctly, what can be rolled back, and what must not be changed without human approval. Do not rely on remembered pricing, limits, UI labels, commands, or setup mechanics; route those to current vendor docs.
```

<!-- VCB:END_SECTION id=tool.netlify.ai_coach -->

## Shortcut Reality

### The shortcut people take

They add Netlify because it appears to solve a whole category quickly, then accept the default setup, polished artifact, dashboard status, generated integration, or green check as proof.
...
- `netlify.docs.overview` — official vendor documentation anchor for Netlify facts and volatile setup/product mechanics.
- `netlify.docs.deploy_overview` — official vendor documentation anchor for Netlify facts and volatile setup/product mechanics.
- `netlify.docs.functions_overview` — official vendor documentation anchor for Netlify facts and volatile setup/product mechanics.
- `netlify.docs.environment_variables` — official vendor documentation anchor for Netlify facts and volatile setup/product mechanics.
