<!-- VCB:BEGIN_TOPIC id=tool.vercel version=0.1.0 -->
---
version: 0.1.0
...
setup_effort: 'medium: Git-connected deployment can be quick, but domains, env vars, preview/production
  separation, functions, and rollback posture need explicit decisions'
maintenance_effort: 'medium-high: hosting, build settings, runtime functions, environment
  variables, branch/deployment rules, and observability need ongoing review'
debugging_effort: 'medium-high: failures may involve build logs, framework adapters, environment
  variables, serverless/edge behavior, domains, caching, or deployment promotion'
lock_in_risk: 'moderate-high: deployment platform conventions, preview workflow, runtime choices,
  analytics/observability, and integrations can become part of the app architecture'
hidden_cost_risk: 'high: easy previews hide costs for environment management, domain ownership,
  function behavior, rollback, observability, and migration'
...
- production-bound web app deployment
- branch/PR preview review loops
- serverless/edge-adjacent app hosting
- deployment evidence before release
not_for:
- unreviewed production promotion
- unclear environment-variable ownership
- stable prose about current deployment limits or pricing
- apps without rollback or observability expectations
- secret-heavy experiments without environment separation
integrates_with_codex:
- GitHub PR and branch review
- GitHub Actions checks
- Sentry error evidence
- PostHog analytics evidence
- Codex hardening and release-note passes
hidden_costs:
...
obsolete_when:
- Vercel is no longer available, no longer documented by its vendor, or no longer serves its
  stated hosting role.
related_topics:
- tool.github
- tool.github_actions
...
> Summary:
> Vercel is a frontend/application hosting, preview deployments, environment separation, and deployment evidence tool. Use it to make product infrastructure more explicit, not to skip design, review, security, or operational ownership.
...
<!-- VCB:BEGIN_SECTION id=tool.vercel.human -->
## 1. For the Human: Plain-Language Concept

### What this tool is

Vercel is for frontend/application hosting, preview deployments, environment separation, and deployment evidence. It belongs in the tool stack when the project needs that infrastructure role to be explicit and reviewable.
...
|---|---|
| Setup effort | medium: Git-connected deployment can be quick, but domains, env vars, preview/production separation, functions, and rollback posture need explicit decisions |
| Maintenance effort | medium-high: hosting, build settings, runtime functions, environment variables, branch/deployment rules, and observability need ongoing review |
| Debugging effort | medium-high: failures may involve build logs, framework adapters, environment variables, serverless/edge behavior, domains, caching, or deployment promotion |
| Lock-in risk | moderate-high: deployment platform conventions, preview workflow, runtime choices, analytics/observability, and integrations can become part of the app architecture |
| Hidden cost risk | high: easy previews hide costs for environment management, domain ownership, function behavior, rollback, observability, and migration |
...
<!-- VCB:END_SECTION id=tool.vercel.human -->

<!-- VCB:BEGIN_SECTION id=tool.vercel.ai_coach -->
## 2. For the AI Coach: How to Teach This to Your Human

### Teaching objective

Teach `tool.vercel` as a production-infrastructure tool card. The human should use it to expose risk, evidence, and ownership, not to outsource judgment to a vendor dashboard or generated integration.
...
```text
Use `tool.vercel` only as a bounded tool in this project. First identify what it can touch, what evidence proves it is configured correctly, what can be rolled back, and what must not be changed without human approval. Do not rely on remembered pricing, limits, UI labels, or setup mechanics; route those to current vendor docs.
```

<!-- VCB:END_SECTION id=tool.vercel.ai_coach -->

## Shortcut Reality

### The shortcut people take

They add Vercel because it appears to solve a whole infrastructure category quickly, then accept the default setup or generated integration as production evidence.
...
- `vercel.docs.overview` — official vendor documentation anchor.
- `vercel.docs.deployments` — official vendor documentation anchor.
- `vercel.docs.observability` — official vendor documentation anchor.
- `vercel.docs.analytics` — official vendor documentation anchor.
