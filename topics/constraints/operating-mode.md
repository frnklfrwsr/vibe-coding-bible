<!-- VCB:BEGIN_TOPIC id=vcb.constraints.operating_mode version=0.1.0 -->
---
version: 0.1.0
...
id: vcb.constraints.operating_mode
title: Operating Mode Constraint
type: concept
...
- budget decisions should account for usage, attention, recovery, and maintenance
  cost
likely_to_change:
- current Codex surfaces and delegation features
- model and reasoning-mode names
- current usage limits, plan packaging, and UI affordances
obsolete_when:
- Codex exposes a stable, automatic operating-mode selector that reliably accounts
  for risk, permissions, budget, phase, and review evidence.
related_topics:
...
<!-- VCB:BEGIN_SECTION id=vcb.constraints.operating_mode.structure_notice -->
...
<!-- VCB:END_SECTION id=vcb.constraints.operating_mode.structure_notice -->
...
<!-- VCB:BEGIN_SECTION id=vcb.constraints.operating_mode.human -->
## 1. For the Human: Plain-Language Concept

### What this means

An operating mode is the default way you run Codex for a task: local or cloud, small or broad task, read-only or write access, continuous supervision or review later.

### Why it matters

The wrong operating mode wastes either attention or usage and can expand blast radius. A tiny local change does not need a huge cloud delegation; a cross-repo production change should not be treated like a casual chat.

### What good looks like

Good: “For this auth fix, use a branch, inspect first, propose a plan, edit only the auth middleware and tests, then show diff and verification.”

### What bad looks like

Bad: “Fix auth” with no surface choice, no branch, no permission boundary, no done-when, and no review evidence.

### Minimal example

Ask Codex: “Choose the safest useful operating mode for this task. Recommend surface, task size, permission level, supervision rhythm, and required evidence before coding. Do not start implementation yet.”
...
<!-- VCB:END_SECTION id=vcb.constraints.operating_mode.human -->

<!-- VCB:BEGIN_SECTION id=vcb.constraints.operating_mode.ai_coach -->
## 2. For the AI Coach: How to Teach This to Your Human
...
```text
Choose the safest useful operating mode for this task. Recommend surface, task size, permission level, supervision rhythm, and required evidence before coding. Do not start implementation yet.
Return:
...
<!-- VCB:END_SECTION id=vcb.constraints.operating_mode.ai_coach -->
...
Use the smallest Codex surface and permission profile that can produce reviewable evidence. Upgrade surface or throughput only when the task shape justifies it. The cheap path is not “skip checks”; it is “pay only for the context, edits, and verification that prove the current slice.”
...
<!-- VCB:END_TOPIC id=vcb.constraints.operating_mode -->
