<!-- VCB:BEGIN_TOPIC id=tool.auth0 version=0.1.0 -->
---
version: 0.1.0
...
setup_effort: 'medium-high: quickstarts help, but application type, API/resource-server model,
  OAuth/OIDC flow, tenant settings, user stores, and org/enterprise choices need architecture
  review'
maintenance_effort: 'high: IAM settings, applications/APIs, connections, rules/actions, tenants,
  enterprise identity, token handling, and policies need sustained governance'
debugging_effort: 'high: failures may involve redirects, callbacks, OIDC/OAuth flow choice,
  token audience/scope, API validation, tenant settings, roles, or enterprise connections'
lock_in_risk: 'high: tenant configuration, identity data, enterprise connection setup, hosted
  login customization, and authorization model can be costly to migrate'
hidden_cost_risk: 'high: identity is not just login; authorization, token validation, tenant
  governance, auditability, support, and compliance become recurring costs'
...
- OIDC/OAuth-centered app/API access
- multi-application auth architecture
- SSO/enterprise connection evaluation
- auth architecture with explicit tenant governance
not_for:
- tiny prototypes that only need fake auth
- treating hosted login as complete authorization
- unreviewed token/audience/scope validation
- stable prose about current plan/feature packaging
- security-sensitive IAM without human ownership
integrates_with_codex:
- Vercel or backend app deployment
- Supabase/backend API authorization
- Stripe customer/account mapping
- Sentry/PostHog identity context
- Codex Security threat modeling
hidden_costs:
...
obsolete_when:
- Auth0 is no longer available, no longer documented by its vendor, or no longer serves its
  stated auth role.
related_topics:
- tool.clerk
- tool.supabase
...
> Summary:
> Auth0 is a identity and access-management platform for authentication, authorization, applications, APIs, and enterprise/B2B identity posture tool. Use it to make product infrastructure more explicit, not to skip design, review, security, or operational ownership.
...
<!-- VCB:BEGIN_SECTION id=tool.auth0.human -->
## 1. For the Human: Plain-Language Concept

### What this tool is

Auth0 is for identity and access-management platform for authentication, authorization, applications, APIs, and enterprise/B2B identity posture. It belongs in the tool stack when the project needs that infrastructure role to be explicit and reviewable.
...
|---|---|
| Setup effort | medium-high: quickstarts help, but application type, API/resource-server model, OAuth/OIDC flow, tenant settings, user stores, and org/enterprise choices need architecture review |
| Maintenance effort | high: IAM settings, applications/APIs, connections, rules/actions, tenants, enterprise identity, token handling, and policies need sustained governance |
| Debugging effort | high: failures may involve redirects, callbacks, OIDC/OAuth flow choice, token audience/scope, API validation, tenant settings, roles, or enterprise connections |
| Lock-in risk | high: tenant configuration, identity data, enterprise connection setup, hosted login customization, and authorization model can be costly to migrate |
| Hidden cost risk | high: identity is not just login; authorization, token validation, tenant governance, auditability, support, and compliance become recurring costs |
...
<!-- VCB:END_SECTION id=tool.auth0.human -->

<!-- VCB:BEGIN_SECTION id=tool.auth0.ai_coach -->
## 2. For the AI Coach: How to Teach This to Your Human

### Teaching objective

Teach `tool.auth0` as a production-infrastructure tool card. The human should use it to expose risk, evidence, and ownership, not to outsource judgment to a vendor dashboard or generated integration.
...
```text
Use `tool.auth0` only as a bounded tool in this project. First identify what it can touch, what evidence proves it is configured correctly, what can be rolled back, and what must not be changed without human approval. Do not rely on remembered pricing, limits, UI labels, or setup mechanics; route those to current vendor docs.
```

<!-- VCB:END_SECTION id=tool.auth0.ai_coach -->

## Shortcut Reality

### The shortcut people take

They add Auth0 because it appears to solve a whole infrastructure category quickly, then accept the default setup or generated integration as production evidence.
...
- `auth0.docs.get_started` — official vendor documentation anchor.
- `auth0.docs.flows` — official vendor documentation anchor.
- `auth0.docs.universal_login` — official vendor documentation anchor.
- `auth0.docs.apis` — official vendor documentation anchor.
- `auth0.docs.organizations` — official vendor documentation anchor.
