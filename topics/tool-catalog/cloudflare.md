<!-- VCB:BEGIN_TOPIC id=tool.cloudflare version=0.1.0 -->
---
version: 0.1.0
...
evidence_scope: official Cloudflare documentation anchors for developer platform,
  Workers, Pages, DNS, and VCB maintainer synthesis for risk and coaching guidance.
budget_profiles:
- plus_constrained
- pro_high_throughput
- api_pay_as_you_go
...
name: Cloudflare
type: tool_card
category: edge_hosting_security
status: active
...
setup_effort: 'medium-high: DNS, domains, Workers/Pages, environment variables, routes,
  cache, and security controls need explicit source-checked decisions'
maintenance_effort: 'high: domains, routes, certificates, Workers/Pages deployments,
  cache behavior, security posture, and account permissions drift over time'
debugging_effort: 'high: failures may involve DNS propagation, proxy/cache behavior,
  routing, edge runtime behavior, secrets, redirects, or account configuration'
lock_in_risk: 'moderate-high: DNS, edge runtime, CDN/security rules, and Workers/Pages
  conventions can become architectural dependencies'
hidden_cost_risk: 'high: edge convenience can hide DNS, security, cache, migration,
...
codex_integration_value: 'medium-high: useful as evidence infrastructure or implementation
  ...
  acceptance criteria'
best_for:
- DNS, domain, CDN, and edge delivery review
- Cloudflare Workers or Pages projects
- edge-adjacent app deployment and routing
- production traffic protection and redirect/routing evidence
- preview or prototype work that needs an explicit hardening path
not_for:
- blind DNS or proxy changes
- using edge functions as a security proof
- stable prose about current limits, pricing, or runtime mechanics
- production traffic migration without rollback
- real secrets in throwaway edge prototypes
...
obsolete_when:
- Cloudflare is no longer available, no longer documented by its vendor, or no longer
...
> Summary:
> Cloudflare is for edge DNS, CDN, Workers/Pages, security, and deployment-adjacent infrastructure evidence. Use it to make evidence more explicit, not to skip design, review, security, or operational ownership.
...
<!-- VCB:BEGIN_SECTION id=tool.cloudflare.human -->
## 1. For the Human: Plain-Language Concept

### What this tool is

Cloudflare is an edge platform for DNS, CDN/security controls, Workers, Pages, and web-application delivery infrastructure.

### Why it matters

It often sits in front of the app, domain, deployment, or edge function path. A wrong setting can break production traffic, leak behavior, or make a generated deployment look safer than it is.
...
|---|---|
| Setup effort | medium-high: DNS, domains, Workers/Pages, environment variables, routes, cache, and security controls need explicit source-checked decisions |
| Maintenance effort | high: domains, routes, certificates, Workers/Pages deployments, cache behavior, security posture, and account permissions drift over time |
| Debugging effort | high: failures may involve DNS propagation, proxy/cache behavior, routing, edge runtime behavior, secrets, redirects, or account configuration |
| Lock-in risk | moderate-high: DNS, edge runtime, CDN/security rules, and Workers/Pages conventions can become architectural dependencies |
| Hidden cost risk | high: edge convenience can hide DNS, security, cache, migration, incident, and rollback cost |
...
<!-- VCB:END_SECTION id=tool.cloudflare.human -->

<!-- VCB:BEGIN_SECTION id=tool.cloudflare.ai_coach -->
## 2. For the AI Coach: How to Teach This to Your Human

### Coaching stance

Treat `tool.cloudflare` as a tool-selection and evidence-routing card. The goal is not to make the user use Cloudflare; it is to decide whether this surface is the smallest reliable tool for the job and what evidence is required before trusting it.
...
```text
Use `tool.cloudflare` only as a bounded tool in this project. First identify what it can touch, what evidence proves it is configured or used correctly, what can be rolled back, and what must not be changed without human approval. Do not rely on remembered pricing, limits, UI labels, commands, or setup mechanics; route those to current vendor docs.
```

<!-- VCB:END_SECTION id=tool.cloudflare.ai_coach -->

## Shortcut Reality

### The shortcut people take

They add Cloudflare because it appears to solve a whole category quickly, then accept the default setup, polished artifact, dashboard status, generated integration, or green check as proof.
...
- `cloudflare.docs.developer_overview` — official vendor documentation anchor for Cloudflare facts and volatile setup/product mechanics.
- `cloudflare.docs.workers` — official vendor documentation anchor for Cloudflare facts and volatile setup/product mechanics.
- `cloudflare.docs.pages` — official vendor documentation anchor for Cloudflare facts and volatile setup/product mechanics.
- `cloudflare.docs.dns` — official vendor documentation anchor for Cloudflare facts and volatile setup/product mechanics.
...
<!-- VCB:END_TOPIC id=tool.cloudflare -->
