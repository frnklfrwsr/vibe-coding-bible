<!-- VCB:BEGIN_TOPIC id=tool.posthog version=0.1.0 -->
---
version: 0.1.0
...
setup_effort: 'medium: event capture can start quickly, but event taxonomy, identity, privacy,
  consent, feature-flag ownership, and analysis questions must be designed'
maintenance_effort: 'medium-high: events, properties, funnels, flags, replays, dashboards,
  retention, privacy, and experiment logic drift with product changes'
debugging_effort: 'medium-high: failures may involve missing events, identity merging, over/under-capture,
  privacy filters, flag targeting, replay gaps, or misleading funnels'
lock_in_risk: 'moderate-high: event taxonomy, feature flags, dashboards, replay workflows,
  and experiment history can become product decision infrastructure'
hidden_cost_risk: 'high: bad events create false product conclusions; overcollection creates
  privacy risk; flags can become stale production behavior'
...
- session replay for UX evidence
- feature flags and experiments
- user-behavior evidence after launch
- feature-adoption and retention review
not_for:
- tracking without consent/privacy review
- using dashboards as causal proof without product context
- permanent stale feature flags
- stable prose about current event/pricing limits
- collecting secrets or sensitive content in replay
integrates_with_codex:
- Vercel hosted apps
- Clerk/Auth0 user identity context
- Stripe revenue/customer events
- Sentry error correlation
- Codex product-doc and QA loops
hidden_costs:
...
obsolete_when:
- PostHog is no longer available, no longer documented by its vendor, or no longer serves
  its stated analytics role.
related_topics:
- tool.vercel
- tool.clerk
- tool.auth0
...
> Summary:
> PostHog is a product analytics, events, funnels, session replay, feature flags, experiments, and user-behavior evidence tool. Use it to make product infrastructure more explicit, not to skip design, review, security, or operational ownership.
...
<!-- VCB:BEGIN_SECTION id=tool.posthog.human -->
## 1. For the Human: Plain-Language Concept

### What this tool is

PostHog is for product analytics, events, funnels, session replay, feature flags, experiments, and user-behavior evidence. It belongs in the tool stack when the project needs that infrastructure role to be explicit and reviewable.
...
|---|---|
| Setup effort | medium: event capture can start quickly, but event taxonomy, identity, privacy, consent, feature-flag ownership, and analysis questions must be designed |
| Maintenance effort | medium-high: events, properties, funnels, flags, replays, dashboards, retention, privacy, and experiment logic drift with product changes |
| Debugging effort | medium-high: failures may involve missing events, identity merging, over/under-capture, privacy filters, flag targeting, replay gaps, or misleading funnels |
| Lock-in risk | moderate-high: event taxonomy, feature flags, dashboards, replay workflows, and experiment history can become product decision infrastructure |
| Hidden cost risk | high: bad events create false product conclusions; overcollection creates privacy risk; flags can become stale production behavior |
...
<!-- VCB:END_SECTION id=tool.posthog.human -->

<!-- VCB:BEGIN_SECTION id=tool.posthog.ai_coach -->
## 2. For the AI Coach: How to Teach This to Your Human

### Teaching objective

Teach `tool.posthog` as a production-infrastructure tool card. The human should use it to expose risk, evidence, and ownership, not to outsource judgment to a vendor dashboard or generated integration.
...
```text
Use `tool.posthog` only as a bounded tool in this project. First identify what it can touch, what evidence proves it is configured correctly, what can be rolled back, and what must not be changed without human approval. Do not rely on remembered pricing, limits, UI labels, or setup mechanics; route those to current vendor docs.
```

<!-- VCB:END_SECTION id=tool.posthog.ai_coach -->

## Shortcut Reality

### The shortcut people take

They add PostHog because it appears to solve a whole infrastructure category quickly, then accept the default setup or generated integration as production evidence.
...
- `posthog.docs.overview` — official vendor documentation anchor.
- `posthog.docs.product_analytics` — official vendor documentation anchor.
- `posthog.docs.events` — official vendor documentation anchor.
- `posthog.docs.feature_flags` — official vendor documentation anchor.
- `posthog.docs.session_replay` — official vendor documentation anchor.
