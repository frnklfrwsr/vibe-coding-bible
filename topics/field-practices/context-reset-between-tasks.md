<!-- VCB:BEGIN_TOPIC id=vcb.field.context_reset_between_tasks version=1.0.1-post-release.issue7 -->
---
...
id: vcb.field.context_reset_between_tasks
title: Context Reset Between Tasks
type: field_practice
status: active
...
durable_principles:
- Context hygiene reduces stale assumptions and keeps each work order tied to current files
...
obsolete_when:
- Context compaction and retrieval become transparent enough that stale or irrelevant working
  memory is no longer a practical failure source.
related_topics:
- vcb.prompting.context_management
- vcb.failure.context_pollution
- vcb.shortcut.context_dumping
- vcb.field.lessons_file_loop
...
> Summary:
> Candidate field practice: Reset, summarize, or start a fresh session between unrelated tasks so stale assumptions, discarded plans, and noisy intermediate output do not steer the next task. This card labels the practice, gives safe trial conditions, and prevents the tactic from being treated as official best practice before evidence supports promotion.
...
<!-- VCB:BEGIN_SECTION id=vcb.field.context_reset_between_tasks.human -->
## 1. For the Human: Plain-Language Concept

### What this practice is

Reset, summarize, or start a fresh session between unrelated tasks so stale assumptions, discarded plans, and noisy intermediate output do not steer the next task.
...
Good: Before a new task, the human sends a fresh context packet: goal, files, current status, constraints, and what not to reuse from the last task.

### What bad looks like

Bad: A long thread moves from bug fix to feature to refactor while the model keeps reusing outdated assumptions from earlier turns.

### Safe trial

After any unrelated task boundary, start a new thread or ask for a short handoff summary, then require the next agent to inspect current repo state before editing.
...
<!-- VCB:END_SECTION id=vcb.field.context_reset_between_tasks.human -->

<!-- VCB:BEGIN_SECTION id=vcb.field.context_reset_between_tasks.ai_coach -->
## 2. For the AI Coach: How to Teach This to Your Human
...
Practice: Context Reset Between Tasks
Claim: Reset, summarize, or start a fresh session between unrelated tasks so stale assumptions, discarded plans, and noisy intermediate output do not steer the next task.
...
<!-- VCB:END_SECTION id=vcb.field.context_reset_between_tasks.ai_coach -->

## Shortcut Reality

### Ideal practice

Start from the durable principle: Context hygiene reduces stale assumptions and keeps each work order tied to current files and acceptance criteria.
...
- fresh context packet beats conversation memory
- bounded trial task
...
- The durable principle is stable: Context hygiene reduces stale assumptions and keeps each work order tied to current files and acceptance criteria.
...
- Context compaction and retrieval become transparent enough that stale or irrelevant working memory is no longer a practical failure source.
...
<!-- VCB:END_TOPIC id=vcb.field.context_reset_between_tasks -->
