<!-- VCB:BEGIN_TOPIC id=tool.stripe version=0.1.0 -->
---
version: 0.1.0
...
setup_effort: 'medium-high: hosted checkout can be quick, but products/prices, customers,
  subscriptions, webhook fulfillment, taxes, disputes, and failure states need explicit design'
maintenance_effort: 'high: payment state, subscription lifecycle, webhook idempotency, customer
  support, taxes, refunds, disputes, and entitlement logic require ongoing operational review'
debugging_effort: 'high: failures may involve asynchronous webhooks, payment intent/subscription
  state, entitlement drift, test/live mode, customer records, or fulfillment side effects'
lock_in_risk: 'high: billing objects, webhook semantics, customer/payment records, portal
  behavior, subscriptions, and reconciliation can become core business infrastructure'
hidden_cost_risk: 'very high: real money creates support, refund, dispute, tax, compliance,
  entitlement, and incident-response costs beyond the API call'
...
- subscriptions and billing workflows
- webhook-driven fulfillment
- customer billing portal flows
- real-money evidence before production launch
not_for:
- fake-success payment demos promoted to production
- fulfillment without webhook verification
- hardcoded payment status assumptions
- stable prose about current fees or plan limits
- unreviewed auth/payment coupling
integrates_with_codex:
- Vercel/Supabase backend webhooks
- Clerk/Auth0 customer identity mapping
- Sentry error evidence
- PostHog funnel/revenue analytics
- Codex Security payment-flow review
hidden_costs:
...
obsolete_when:
- Stripe is no longer available, no longer documented by its vendor, or no longer serves its
  stated payments role.
related_topics:
- tool.supabase
- tool.clerk
- tool.auth0
...
> Summary:
> Stripe is a payments, subscriptions, billing objects, checkout, webhooks, and payment-state evidence for real-money workflows tool. Use it to make product infrastructure more explicit, not to skip design, review, security, or operational ownership.
...
<!-- VCB:BEGIN_SECTION id=tool.stripe.human -->
## 1. For the Human: Plain-Language Concept

### What this tool is

Stripe is for payments, subscriptions, billing objects, checkout, webhooks, and payment-state evidence for real-money workflows. It belongs in the tool stack when the project needs that infrastructure role to be explicit and reviewable.
...
|---|---|
| Setup effort | medium-high: hosted checkout can be quick, but products/prices, customers, subscriptions, webhook fulfillment, taxes, disputes, and failure states need explicit design |
| Maintenance effort | high: payment state, subscription lifecycle, webhook idempotency, customer support, taxes, refunds, disputes, and entitlement logic require ongoing operational review |
| Debugging effort | high: failures may involve asynchronous webhooks, payment intent/subscription state, entitlement drift, test/live mode, customer records, or fulfillment side effects |
| Lock-in risk | high: billing objects, webhook semantics, customer/payment records, portal behavior, subscriptions, and reconciliation can become core business infrastructure |
| Hidden cost risk | very high: real money creates support, refund, dispute, tax, compliance, entitlement, and incident-response costs beyond the API call |
...
<!-- VCB:END_SECTION id=tool.stripe.human -->

<!-- VCB:BEGIN_SECTION id=tool.stripe.ai_coach -->
## 2. For the AI Coach: How to Teach This to Your Human

### Teaching objective

Teach `tool.stripe` as a production-infrastructure tool card. The human should use it to expose risk, evidence, and ownership, not to outsource judgment to a vendor dashboard or generated integration.
...
```text
Use `tool.stripe` only as a bounded tool in this project. First identify what it can touch, what evidence proves it is configured correctly, what can be rolled back, and what must not be changed without human approval. Do not rely on remembered pricing, limits, UI labels, or setup mechanics; route those to current vendor docs.
```

<!-- VCB:END_SECTION id=tool.stripe.ai_coach -->

## Shortcut Reality

### The shortcut people take

They add Stripe because it appears to solve a whole infrastructure category quickly, then accept the default setup or generated integration as production evidence.
...
- `stripe.docs.billing_quickstart` — official vendor documentation anchor.
- `stripe.docs.webhooks` — official vendor documentation anchor.
- `stripe.docs.subscriptions` — official vendor documentation anchor.
- `stripe.docs.checkout` — official vendor documentation anchor.
